...
class Post_Block extends Post_Abstract {
	...
	public function get_primary_category( $post_id ) {
		...
		$cat_id          = apply_filters( 'jeg_primary_category', $category_id, $post_id );
		$category_enable = ! ! $this->attributes['categoryEnabled'];
		$category        = '';
	...
	public function post_meta( $post ) {
		$meta        = null;
		$meta_enable = ! ! $this->attributes['metaEnabled'];

		if ( $meta_enable ) {
			$author_output = null;
			$date_output   = null;

			$author_enable = ! ! $this->attributes['metaAuthorEnabled'];
			$date_enable   = ! ! $this->attributes['metaDateEnabled'];
		...
		return apply_filters( 'guten_post_block_meta', $meta, $post, $this );
	}
	...
	protected function get_excerpt( $post ) {
		$excerpt        = null;
		$excerpt_enable = ! ! $this->attributes['excerptEnabled'];
	...
	protected function get_readmore( $post ) {
		$readmore        = null;
		$readmore_enable = ! ! $this->attributes['readmoreEnabled'];
	...
	public function guten_get_comments_number( $post_id = 0 ) {
		...
		return apply_filters( 'guten_get_comments_number', $comments_number, $post_id );
	}
	...
	protected function get_comment_bubble( $post ) {
		$comment        = null;
		$comment_enable = ! ! $this->attributes['commentEnabled'];
